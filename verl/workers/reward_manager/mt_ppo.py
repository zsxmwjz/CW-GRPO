...
from verl.workers.reward_manager.llm_judge import client, JUDGE_PROMPT
...
@register("mt-ppo")
class MTPPORewardManager(AbstractRewardManager):
    def __init__(
        self,
        tokenizer,
        num_examine: int,
        ...
        **kwargs: Any,
        ...
        """
        Initialize the MTPPORewardManager instance.
    ...
    def __call__(
        ...
        for i in range(len(data)):
            ...
            # Compute format rewards and search penalties
            format_rewards = []
            search_penalties = []
            round_cnt = 0
            for msg in messages:
                if msg.get("role") == "assistant":
                    ...
                    if tool_calls is None:
                        tool_calls = []
                    if len(tool_calls) == 0: # the last round
                        if not re.search(r"<answer>(.*)</answer>", msg["content"], re.DOTALL):
                            format_rewards.append(-1)
                        else:
                            format_rewards.append(1)
                    elif len(tool_calls) == 1:
                        format_rewards.append(0.1)
                    else:
                        format_rewards.append(-0.2)
                    round_cnt += 1
                    search_penalties.append(0.1 * round_cnt)
            ...
            data_items_info.append({
                ...
                "search_penalties": search_penalties,
        ...
        for i, item_info in enumerate(data_items_info):
            ...
            search_penalties = item_info["search_penalties"]
            ...
            for idx, token_pos in enumerate(true_indices):
                if idx < len(judge_results):
                    reward = 0.3 * judge_results[idx]["retrieval_reward"] + format_rewards[idx] - search_penalties[idx]
                elif len(judge_results) == 0:
                    reward = 0
                else:
                    reward = 1 if answer_reward else 0.2 if format_rewards[idx] == 1 else -1
